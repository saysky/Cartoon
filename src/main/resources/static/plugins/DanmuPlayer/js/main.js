...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$ctrlMain.append('<div class=" send-btn  ctrl-btn"  id="danmu-send-btn" >发送 ></div>');
        ...
        $("body").append('<div id="' + this.id.slice(1, this.id.length) + 'fontTip"  hidden="false">' +
        ...
        this.getDanmu = function () {
            $.get(that.options.urlToGetDanmu, function (data, status) {
                ...
                try {
                    let danmuFromSql = eval(data);
                    $(that.id + ' .danmu-div').danmu("addDanmu", danmuFromSql)
                } catch (e) {
                }
                // by 言曌 修改bug 2021年03月07日
